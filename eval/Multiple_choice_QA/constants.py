...
with open("/ai/home/wwz/LLaMA-Factory/examples/train_lora/lora_eval.yaml", "r") as f:
    config_data = yaml.safe_load(f)
    config = Dict(config_data)  # 转换为对象（可选）

RUNNING_LOG = "running_log.txt"
SUBJECTS_TASK={
'mmlu_test' : ["Average", "STEM", "Social Sciences", "Humanities", "Other"],
'medQA_test' : ["卫生法规","第四部分运动系统疾病","医学微生物学","第一部分历年真题","第1篇呼吸系统","第3篇消化系统","第六部分儿科疾病","第2篇心血管系统","生理学",
    "生物化学","药理学","医学伦理学","医学免疫学","第一部分血液系统疾病","第七部分传染病、性传播疾病","第三部分精神神经系统疾病","第八部分其他疾病","病理学",
    "第二部分代谢、内分泌系统疾病","第五部分风湿免疫性疾病","第5篇女性生殖系统","医学心理学","第4篇泌尿系统","预防医学","第二部分模拟试题","Average"],
'cancerzh_test' : ["肿瘤内科", "肿瘤学","Average"],
'medQAen_test' : ["medQA","Average"],
'medQAzh_test' : ["medQA","Average"]
                }

def get_subjects_by_task(task_name):
    """获取指定任务对应的学科列表"""
    return SUBJECTS_TASK.get(task_name, [])  # 默认返回空列表

SUBJECTS = get_subjects_by_task(config.task)
