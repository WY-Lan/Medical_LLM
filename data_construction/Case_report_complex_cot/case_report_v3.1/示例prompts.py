...
Extract the precise diagnosis information from the following clinical case and discussion. 
Include only factual diagnostic information that is explicitly stated in the text. 
Do not add any interpretations or information not present in the original content.

CASE CONTENT: {case_content}

DISCUSSION: {discussion_content}

Focus on extracting the following diagnostic elements:
1. Primary diagnosis: The main condition identified in the patient
2. Differential diagnoses: Alternative conditions that were considered
3. Diagnostic reasoning: Key findings that led to the diagnosis
4. Disease staging or classification: If the condition was classified by severity or stage
5. Diagnostic certainty: Whether the diagnosis was confirmed, probable, or suspected
6. Comorbidities: Additional diagnoses mentioned alongside the primary condition
7. Etiology: Identified causes of the condition when explicitly stated

Provide the diagnosis in a concise, clear format. Include all diagnostic information in the order it appears. 
Use medical terminology as presented in the original text. 
If multiple diagnoses were made, include all of them. 
Do not include treatment information or prognosis in this section.

Format your response as a valid JSON with a single key 'diagnosis' and nothing else:
{{
  "diagnosis": "extracted diagnosis text here"
}}
"""

# 用于从特定治疗章节提取治疗信息的提示模板 - 旧版
TREATMENT_FROM_SECTIONS_TEMPLATE = """
You are a medical treatment specialist with expertise in therapeutic interventions. 
Extract all treatment information from the following treatment-specific sections of a clinical case. 
Focus on the information contained within these dedicated treatment sections. 
Include only factual treatment information that is explicitly stated. 
Maintain the original clinical terminology and wording where possible.
...
1. Identify the natural treatment phases/stages present in this specific case based on the content.
2. Don't force the content into predefined stages - let the case's natural progression guide your organization.
3. For each identified treatment phase, provide a descriptive name that reflects its nature (e.g., 'Initial Assessment', 'Surgical Intervention', 'Antibiotic Therapy', 'Post-operative Care', etc.)
4. Organize all treatment information chronologically within each identified phase.
5. Include temporal markers (dates, time periods) when mentioned.
6. For each treatment element, include dosages, routes, durations, and specific techniques when mentioned.

Format your response as a structured JSON where:
- Keys are the identified treatment phases (use descriptive but concise names in snake_case)
- Values are detailed descriptions of treatments during those phases

Example format (the actual phases should be derived from content):
{{
  "emergency_stabilization": "Detailed description of emergency treatments...",
  "surgical_management": "Detailed description of surgical procedures...",
  "post_surgical_care": "Detailed description of post-surgical treatments...",
  "discharge_planning": "Detailed description of discharge treatments..."
}}

If you can't identify distinct phases, use a single key 'overall_treatment' with all information.
"""

# 用于从整个内容提取治疗信息的提示模板 - 旧版
TREATMENT_FROM_CONTENT_TEMPLATE = """
You are a medical treatment specialist with expertise in therapeutic interventions. 
Extract ALL treatment information from the following clinical case text, even if treatment details 
are scattered throughout different parts of the text. 
Include only factual treatment information that is explicitly stated. 
Maintain the original clinical terminology and wording where possible.
