...
class GPT:
    ...
    def retry_call(self, content, additional_args={"max_tokens": 6144}):
...
- **'Verification'**: At this stage, you verify the conclusion from the "Final Conclusion" step. If the conclusion holds, end the process. If not, return to "Inner Thinking" for further reasoning. No title is required here. his section should be placed under **"Final Conclusion"**.

The output format must strictly follow the JSON structure below, Strictly follow the JSON structure below, and all content within the JSON fields should be written in **Chinese**:
```json
{{
  "CoT": [
...
2. **Final Conclusion**: Summarize the correct reasoning from all previous 'Inner Thinking' steps and provide the final answer. No title is needed for this section, and this part is required, this part is required, and this part is required.
...
### Output Format
Strictly follow the JSON structure below. Strictly follow the JSON structure below. Strictly follow the JSON structure below.  All content within the JSON fields must be written in **Chinese**. You do not need to repeat your previous reasoning. Begin directly from the next 'Verification' stage.
...
1. **Inner Thinking**: Break down the reasoning process into multiple concise steps. Each step should start with a brief title to clarify its purpose.t
2. **Final Conclusion**: Summarize the correct reasoning from all previous 'Inner Thinking' steps and provide the final answer. No title is needed for this section, and this part is required, this part is required, and this part is required.
3. **Verification**: Verify the accuracy of the "Final Conclusion". If it holds, conclude the process. Otherwise, return to "Inner Thinking" for further refinement.
...
### Output Format
Strictly follow the JSON structure below. Strictly follow the JSON structure below. Strictly follow the JSON structure below. Must be output in JSON structure. All content within the JSON fields must be written in **Chinese**. You do not need to repeat your previous reasoning. Begin directly from the next 'Verification' stage.
...
Directly return the modified natural thinking in JSON format and put it in the **NaturalReasoning** field as follows:
```json
{{
  "NaturalReasoning": "..."
}}
...
def parse_gpt_response_reformat(response):
    try:
        ...
        da = json.loads(cleaned_response)

        # 检查NaturalReasoning是否是字符串，且包含换行符
...
def main():
    ...
    parser.add_argument("--data_path", type=str, default="data/yc_train_data.json",help="Path to the input JSON data file.")
    parser.add_argument("--model_name", type=str, default="gpt-4o-mini", help="Name of the GPT model to use.")
    ...
    parser.add_argument("--num_process", type=int, default=128, help="Number of parallel processes.")
    ...
    save_dir = f'output_data/{args.model_name}/{task_name}'
